...
PORTABLEAPI(objhld_t) objallo(int size, objinitfn_t initializer, objuninitfn_t unloader, const void *context, unsigned int ctxsize);
/* obsolete implementation, simple way to allocate a object, new application should use @objallo3 instead of it  */
PORTABLEAPI(objhld_t) objallo2(int size);
...
PORTABLEAPI(int) objrefr2(objhld_t hld, void **output); /* reference object, return the size of object context on success */
