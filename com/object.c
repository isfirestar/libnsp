...
#define INCREMENT(n)    InterlockedIncrement(n)
#define DECREMENT(n)     InterlockedDecrement(n)
...
#else
static struct _object_manager g_objmgr = {
    .roottab = { NULL }, 0, PTHREAD_MUTEX_INITIALIZER,
};
#endif
...
PORTABLEIMPL(objhld_t)
objallo3(const struct objcreator *creator)
{
    object_t *obj;
    ...
    obj = (object_t *) malloc(creator->size + sizeof(object_t));
    if (!obj) {
        return INVALID_OBJHLD;
    }
